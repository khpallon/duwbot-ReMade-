...
let ChatClient = ChatClient_1 = class ChatClient extends typed_event_emitter_1.EventEmitter {
    ...
    constructor(config = {}) {
        ...
        this._ircClient.registerMessageType(ChatMessage_1.ChatMessage);
        ...
        this._ircClient.onPrivmsg((channel, user, text, msg) => {
            if (user !== 'jtv') {
                this.emit(this.onMessage, (0, userUtil_1.toUserName)(channel), user, text, msg);
            }
        });
        this._ircClient.onAction((channel, user, text, msg) => {
            this.emit(this.onAction, (0, userUtil_1.toUserName)(channel), user, text, msg);
        });
        this.addInternalListener(this._onJoinResult, (channel, _, error) => {
            if (error) {
                this.emit(this.onJoinFailure, (0, userUtil_1.toUserName)(channel), error);
            }
            else {
        ...
        this._ircClient.onTypedMessage(ClearChat_1.ClearChat, msg => {
            ...
            const broadcasterName = (0, userUtil_1.toUserName)(channel);
            if (user) {
        ...
        this._ircClient.onTypedMessage(ClearMsg_1.ClearMsg, msg => {
            ...
            this.emit(this.onMessageRemove, (0, userUtil_1.toUserName)(channel), targetMessageId, msg);
        });
        this._ircClient.onTypedMessage(ircv3_1.MessageTypes.Commands.ChannelJoin, ({ prefix, channel }) => {
            this.emit(this.onJoin, (0, userUtil_1.toUserName)(channel), prefix.nick);
        });
        this._ircClient.onTypedMessage(ircv3_1.MessageTypes.Commands.ChannelPart, ({ prefix, channel }) => {
            this.emit(this.onPart, (0, userUtil_1.toUserName)(channel), prefix.nick);
        });
        this._ircClient.onTypedMessage(RoomState_1.RoomState, ({ channel, tags }) => {
            let isInitial = false;
        ...
        this._ircClient.onTypedMessage(ircv3_1.MessageTypes.Commands.Notice, async ({ target: channel, text, tags }) => {
            const messageType = tags.get('msg-id');
            switch (messageType) {
                // emote only
                case 'emote_only_on': {
                    this.emit(this.onEmoteOnly, (0, userUtil_1.toUserName)(channel), true);
                    break;
                }
                case 'emote_only_off': {
                    this.emit(this.onEmoteOnly, (0, userUtil_1.toUserName)(channel), false);
                    break;
                }
                ...
                // unique chat
                case 'r9k_on': {
                    this.emit(this.onUniqueChat, (0, userUtil_1.toUserName)(channel), true);
                    break;
                }
                case 'r9k_off': {
                    this.emit(this.onUniqueChat, (0, userUtil_1.toUserName)(channel), false);
                    break;
                }
                // subs only
                case 'subs_on': {
                    this.emit(this.onSubsOnly, (0, userUtil_1.toUserName)(channel), true);
                    break;
                }
                case 'subs_off': {
                    this.emit(this.onSubsOnly, (0, userUtil_1.toUserName)(channel), false);
                    break;
                }
                case 'cmds_available': {
                    // do we really care?
                ...
                case 'no_permission': {
                    this.emit(this.onNoPermission, (0, userUtil_1.toUserName)(channel), text);
                    break;
                }
                case 'msg_ratelimit': {
                    this.emit(this.onMessageRatelimit, (0, userUtil_1.toUserName)(channel), text);
                    break;
                }
                case 'msg_duplicate':
                case 'msg_emoteonly':
                ...
                case 'msg_channel_blocked': {
                    this.emit(this.onMessageFailed, (0, userUtil_1.toUserName)(channel), messageType);
                    break;
                }
                case undefined: {
