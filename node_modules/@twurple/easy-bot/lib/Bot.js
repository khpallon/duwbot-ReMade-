...
const AnnouncementEvent_1 = require("./events/AnnouncementEvent");
const BanEvent_1 = require("./events/BanEvent");
const BitsBadgeUpgradeEvent_1 = require("./events/BitsBadgeUpgradeEvent");
const ChatClearEvent_1 = require("./events/ChatClearEvent");
const CommunityPayForwardEvent_1 = require("./events/CommunityPayForwardEvent");
const CommunitySubEvent_1 = require("./events/CommunitySubEvent");
const EmoteOnlyToggleEvent_1 = require("./events/EmoteOnlyToggleEvent");
const FollowersOnlyToggleEvent_1 = require("./events/FollowersOnlyToggleEvent");
const GiftPaidUpgradeEvent_1 = require("./events/GiftPaidUpgradeEvent");
const JoinEvent_1 = require("./events/JoinEvent");
const JoinFailureEvent_1 = require("./events/JoinFailureEvent");
const LeaveEvent_1 = require("./events/LeaveEvent");
const MessageEvent_1 = require("./events/MessageEvent");
const MessageRemoveEvent_1 = require("./events/MessageRemoveEvent");
const PrimePaidUpgradeEvent_1 = require("./events/PrimePaidUpgradeEvent");
const RaidCancelEvent_1 = require("./events/RaidCancelEvent");
const RaidEvent_1 = require("./events/RaidEvent");
const SlowModeToggleEvent_1 = require("./events/SlowModeToggleEvent");
const StandardPayForwardEvent_1 = require("./events/StandardPayForwardEvent");
const SubEvent_1 = require("./events/SubEvent");
const SubGiftEvent_1 = require("./events/SubGiftEvent");
const SubsOnlyToggleEvent_1 = require("./events/SubsOnlyToggleEvent");
const UniqueChatToggleEvent_1 = require("./events/UniqueChatToggleEvent");
const WhisperEvent_1 = require("./events/WhisperEvent");
/**
 * Twitch chatbots made easy.
 *
 * @meta category main
 */
class Bot extends typed_event_emitter_1.EventEmitter {
    // endregion
    /**
     * Creates a new bot.
     *
    ...
    constructor(config) {
        ...
        this.chat.onMessage(async (channel, user, text, msg) => {
            ...
            if (match === null || match === void 0 ? void 0 : match.command.canExecute(msg.channelId, msg.userInfo.userId)) {
                await match.command.execute(match.params, new BotCommandContext_1.BotCommandContext(this, msg));
            }
            if (match === null || emitCommandMessageEvents) {
                this.emit(this.onMessage, new MessageEvent_1.MessageEvent(channel, user, text, false, msg, this));
            }
        });
        ...
        this.chat.onTimeout((channel, user, duration, msg) => this.emit(this.onTimeout, new BanEvent_1.BanEvent(channel, user, duration, msg, this)));
        this.chat.onBan((channel, user, msg) => this.emit(this.onBan, new BanEvent_1.BanEvent(channel, user, null, msg, this)));
        this.chat.onBitsBadgeUpgrade((channel, user, upgradeInfo, msg) => this.emit(this.onBitsBadgeUpgrade, new BitsBadgeUpgradeEvent_1.BitsBadgeUpgradeEvent(channel, user, upgradeInfo, msg, this)));
        this.chat.onChatClear((channel, msg) => this.emit(this.onChatClear, new ChatClearEvent_1.ChatClearEvent(channel, msg, this)));
        this.chat.onEmoteOnly((channel, enabled) => this.emit(this.onEmoteOnlyToggle, new EmoteOnlyToggleEvent_1.EmoteOnlyToggleEvent(channel, enabled, this)));
        this.chat.onFollowersOnly((channel, enabled, delay) => this.emit(this.onFollowersOnlyToggle, new FollowersOnlyToggleEvent_1.FollowersOnlyToggleEvent(channel, enabled, delay, this)));
        this.chat.onJoin((channel, user) => this.emit(this.onJoin, new JoinEvent_1.JoinEvent(channel, user, this)));
        this.chat.onJoinFailure((channel, reason) => this.emit(this.onJoinFailure, new JoinFailureEvent_1.JoinFailureEvent(channel, reason, this)));
        this.chat.onPart((channel, user) => this.emit(this.onLeave, new LeaveEvent_1.LeaveEvent(channel, user, this)));
        this.chat.onMessageRemove((channel, messageId, msg) => this.emit(this.onMessageRemove, new MessageRemoveEvent_1.MessageRemoveEvent(channel, messageId, msg, this)));
        this.chat.onUniqueChat((channel, enabled) => this.emit(this.onUniqueChatToggle, new UniqueChatToggleEvent_1.UniqueChatToggleEvent(channel, enabled, this)));
        this.chat.onRaid((channel, user, raidInfo, msg) => this.emit(this.onRaid, new RaidEvent_1.RaidEvent(channel, user, raidInfo, msg, this)));
        this.chat.onRaidCancel((channel, msg) => this.emit(this.onRaidCancel, new RaidCancelEvent_1.RaidCancelEvent(channel, msg, this)));
        this.chat.onSlow((channel, enabled, delay) => this.emit(this.onSlowModeToggle, new SlowModeToggleEvent_1.SlowModeToggleEvent(channel, enabled, delay, this)));
        this.chat.onSubsOnly((channel, enabled) => this.emit(this.onSubsOnlyToggle, new SubsOnlyToggleEvent_1.SubsOnlyToggleEvent(channel, enabled, this)));
        this.chat.onSub((channel, user, subInfo, msg) => this.emit(this.onSub, new SubEvent_1.SubEvent(channel, user, subInfo, msg, this)));
        this.chat.onResub((channel, user, subInfo, msg) => this.emit(this.onResub, new SubEvent_1.SubEvent(channel, user, subInfo, msg, this)));
        this.chat.onSubGift((channel, user, subInfo, msg) => this.emit(this.onSubGift, new SubGiftEvent_1.SubGiftEvent(channel, user, subInfo, msg, this)));
        this.chat.onCommunitySub((channel, user, subInfo, msg) => this.emit(this.onCommunitySub, new CommunitySubEvent_1.CommunitySubEvent(channel, subInfo, msg, this)));
        this.chat.onPrimePaidUpgrade((channel, user, subInfo, msg) => this.emit(this.onPrimePaidUpgrade, new PrimePaidUpgradeEvent_1.PrimePaidUpgradeEvent(channel, user, subInfo, msg, this)));
        this.chat.onGiftPaidUpgrade((channel, user, subInfo, msg) => this.emit(this.onGiftPaidUpgrade, new GiftPaidUpgradeEvent_1.GiftPaidUpgradeEvent(channel, user, subInfo, msg, this)));
        this.chat.onStandardPayForward((channel, user, forwardInfo, msg) => this.emit(this.onStandardPayForward, new StandardPayForwardEvent_1.StandardPayForwardEvent(channel, user, forwardInfo, msg, this)));
        this.chat.onCommunityPayForward((channel, user, forwardInfo, msg) => this.emit(this.onCommunityPayForward, new CommunityPayForwardEvent_1.CommunityPayForwardEvent(channel, user, forwardInfo, msg, this)));
        this.chat.onAnnouncement((channel, user, announcementInfo, msg) => this.emit(this.onAnnouncement, new AnnouncementEvent_1.AnnouncementEvent(channel, user, announcementInfo, msg, this)));
        this.chat.onWhisper((user, text, msg) => this.emit(this.onWhisper, new WhisperEvent_1.WhisperEvent(user, text, msg, this)));
        this.chat.onAction((channel, user, text, msg) => this.emit(this.onAction, new MessageEvent_1.MessageEvent(channel, user, text, true, msg, this)));
        // endregion
        this.chat.connect();
    }
    ...
    async deleteMessageById(channel, message) {
        await this.api.asUser(await this._getPreferredUserIdForModAction(channel), async (ctx) => await ctx.moderation.deleteChatMessages(channel, (0, chat_1.extractMessageId)(message)));
    }
    /**
    ...
    async _getBotUserId() {
        ...
        return await (this._botUserIdPromise = this.api
            ...
            .then(tokenInfo => {
            if (!tokenInfo.userId) {
                throw new common_1.HellFreezesOverError('Bot token is not a user token');
            }
    ...
    async _resolveUserId(userNameOrChannel) {
        const userName = (0, chat_1.toUserName)(userNameOrChannel);
